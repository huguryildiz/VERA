...
| ✅ | **31** | Admin large split P2 — EntryControlPage 1565 → 909 orch + 9 komponent (TokenGeneratorCard, SessionHistoryTable, SessionOverviewPanel, SortIcon, 4 modal, entryControlHelpers); OutcomesPage 1535 → 784 orch + 7 komponent (OutcomeRow, OutcomesTable, FrameworkSetupPanel 4-variant, DeleteOutcomeModal, ImportConfirmModal, UnassignFrameworkModal, outcomeHelpers) | 2 page + 16 yeni komponent | ~2h (paralel S32) | %60 | EntryControl warn bandında (909, hard yok); S33 için zemin · [session-31](implementation_reports/session-31-entry-outcomes-split.md) |
| ✅ | **32** | Admin large split P3 — CriteriaPage 1469 → 610 orch + 6 komponent (CriteriaPageHeader, CriteriaFilterPanel, CriteriaEmptyStates, CriteriaTable, CriteriaConfirmModals, criteriaHelpers); GovernanceDrawers 1308 bundle bırakıldı (Option a), `size-ceiling-ok` comment güncellendi — 4 drawer ×200-330 satır coherent bundle | 1 page + 6 yeni komponent + 1 bundle decision | ~2h (paralel S31) | %65 | S33 mid-size evaluation'a hazır · [session-32](implementation_reports/session-32-criteria-governance.md) |
| ✅ | **33** | Mid-size değerlendirmesi (5-sinyal protokolü): JurorsPage 1271→split (JurorsTable+JurorsFilters+JurorsHeader+jurorHelpers), RankingsPage 1126→split (RankingsTable+RankingsHeader+rankingsHelpers), ProjectsPage 1032→split (ProjectsTable+ProjectsFilters+ProjectsHeader+projectHelpers); LandingPage 1183 → 2/5 sinyal, bırakıldı. Test coverage: 39.64% → **40.47%** (+0.83pp); 463 test / 147 dosya. Yeni test dosyaları: auditColumns, juryApi genişleme (+6 fn), periodHelpers, jurorHelpers, projectHelpers, outcomeHelpers. **%50 hedef yapısal nedenle ulaşılamaz** — React component'lar (~40k+ satır) DOM/browser mock olmadan coverage'a girmiyor; pure utility testleri maksimuma ulaştı. | 3 page split + 6 yeni test dosyası + 11 yeni qa-catalog ID | ~2.5h | %75 | Plan tamamlandı · [session-33](implementation_reports/session-33-midsize-split-test-coverage.md) |
...
|---|---|---|---|
| 2026-04-23 | 33 | **S33 tamam — Mid-size split (5-sinyal protokolü) + test coverage build-out:** **5-sinyal değerlendirmesi:** JurorsPage 1271 → 3+ sinyal (split); RankingsPage 1126 → 3+ sinyal (split); ProjectsPage 1032 → 3+ sinyal (split); LandingPage 1183 → 2 sinyal (bırakıldı, `size-ceiling-ok` comment eklendi). **JurorsPage** 1271 → orchestrator + `JurorsHeader` (toolbar+export+filter toggle) + `JurorsFilters` (filter panel) + `JurorsTable` (desktop table + mobile cards + pagination) + `jurorHelpers.js` (formatters + getJurorCell + groupBarColor/Text + mobileScoreStyle). **RankingsPage** 1126 → orchestrator + `RankingsHeader` + `RankingsTable` + `rankingsHelpers.js`. **ProjectsPage** 1032 → orchestrator + `ProjectsHeader` + `ProjectsFilters` + `ProjectsTable` + `projectHelpers.js`. **Test coverage:** 6 yeni test dosyası: `auditColumns.test.js` (audit.cols.01), `juryApi.test.js` genişleme (+6 fn: getJurorById/getJurorEditState/finalizeJurorSubmission/freezePeriodSnapshot/getProjectRankings/submitJuryFeedback), `periodHelpers.test.js` (period.helpers.01), `jurorHelpers.test.js` (juror.helpers.01), `projectHelpers.test.js` (project.helpers.01), `outcomeHelpers.test.js` (outcome.helpers.01). 11 yeni qa-catalog ID eklendi. Coverage: **39.64% → 40.47%** (+0.83pp); 463 test / 147 dosya. **%50 hedef yapısal nedenle ulaşılamadı:** React component'lar (~40k+ satır) DOM/browser mock olmadan v8 coverage'a girmiyor; pure utility testlerle ulaşılabilir maksimum bu noktada. **Build ✅ 12s, 463/463 test.** · [session-33](implementation_reports/session-33-midsize-split-test-coverage.md) | — (plan kapanışı) |
| 2026-04-23 | 32 | **S32 tamam — CriteriaPage split + GovernanceDrawers karar (paralel S31 ile koşuldu):** **CriteriaPage** 1469 → 610 satır orchestrator + 6 yeni komponent (`components/`): `criteriaHelpers.js` (14, `rubricBandClass` + `bandRangeText`), `CriteriaPageHeader` (62, search + filter + export + add button toolbar), `CriteriaFilterPanel` (63, mapping + rubric filter row), `CriteriaEmptyStates` (205, 3 empty-state varyantı + `PendingCriteriaPreview` banner), `CriteriaTable` (466, desktop tablo + mobile kartlar + pagination + inline rubric/mapping editor pattern — CLAUDE.md'de explicit allowed), `CriteriaConfirmModals` (189, `ClearAllCriteria` + `DeleteCriterion` modal bundle). `size-ceiling-ok` yorumu kaldırıldı — 610 satır kabul bandında. S32 sırasında diğer pre-split feature file'ları (CoverageBar, CriteriaManager, CriterionDeleteDialog, CriterionEditor, EditCriteriaDrawer, EditSingleCriterionDrawer, InlineWeightEdit, OutcomePillSelector, ProgrammeOutcomesManagerDrawer, RubricBandEditor, SaveBar, StarterCriteriaDrawer, WeightBudgetBar) dokunulmadı. **GovernanceDrawers** 1308 satırlık bundle olduğu gibi bırakıldı — **Option (a)** seçildi: 4 iç drawer (GlobalSettingsDrawer ~205, ExportBackupDrawer ~258, MaintenanceDrawer ~332, SystemHealthDrawer ~251) her biri coherent 200-330 satır bandında, CLAUDE.md'nin "bundle files that coherently aggregate many small components may legitimately sit at 1000-1500" istisnası tam bu desen. Başlıktaki `size-ceiling-ok` comment "retroactive violation tracked for split" → "coherent 4-drawer governance bundle, S32 decision (Option a)" olarak güncellendi. Option (b) maliyet analizi raporda: 4 yeni dosya + barrel + import churn, reader gain yok. **Build ✅ 6.56s, 278/278 test, `check:js-size` CriteriaPage artık warn listesinde bile değil, diğer tüm check'ler temiz.** · [session-32](implementation_reports/session-32-criteria-governance.md) | Session 33: mid-size pages (JurorsPage, LandingPage, RankingsPage, ProjectsPage) evaluation + test coverage build-out |
| 2026-04-23 | 31 | **S31 tamam — EntryControlPage + OutcomesPage split (paralel S32 ile koşuldu):** **EntryControlPage** 1566 → 909 satır orchestrator + 9 yeni komponent (`components/`): `entryControlHelpers.js` (7 fmt* helper + `toTimestampMs` + `isExpiringSoon`), `SortIcon` (10), `TokenGeneratorCard` (inline token oluşturma formu + QR + kopyala, orchestrator'dan extract), `SessionHistoryTable` (desktop table + portrait cells responsive), `SessionOverviewPanel` (audit/status panel), `SendQrModal` + `NewUserSendModal` + `SendSuccessModal` + `LockWarnModal` (4 modal extract). Orchestrator 909 satır, `check:js-size` warn bandında (hard yok, 800-1000). Pre-split `EntryTokenModal.jsx` / `RevokeTokenModal.jsx` / `JuryEntryControlPanel.jsx` dokunulmadı. **OutcomesPage** 1535 → 784 satır orchestrator + 7 yeni komponent (`components/`): `outcomeHelpers.js` (`coverageBadgeClass` + `coverageLabel` + `naturalCodeSort`), `OutcomeRow` (row component), `OutcomesTable` (desktop + portrait responsive table), `FrameworkSetupPanel` (plan'dan sapma: planlı `OutcomeMappingPanel` yerine — mapping row-level pattern'de, bu panel 4-variant empty state covering), `DeleteOutcomeModal` + `ImportConfirmModal` + `UnassignFrameworkModal`. 784 satır sweet-spot'ta. Pre-split `AddOutcomeDrawer.jsx` / `OutcomeDetailDrawer.jsx` / `useOutcomesExport.js` ve `src/admin/shared/OutcomeEditor.jsx` dokunulmadı. OutcomesPage'in cross-feature import'u `SaveBar from "@/admin/features/criteria/SaveBar"` stabil. CLAUDE.md'deki retroactive violation listesinden OutcomesPage çıkarıldı; EntryControlPage warn listesinde kaldı. **Build ✅ 5.45s, 278/278 test, tüm check'ler temiz.** · [session-31](implementation_reports/session-31-entry-outcomes-split.md) | Session 32: CriteriaPage split + GovernanceDrawers karar (S31 ile paralel çalıştı, ayrı chat) |
