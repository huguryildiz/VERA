...
![QR lock-in animation — access verified](_images/jury/01-arrival.png)

**What this screen shows.** When a juror follows the entry link, VERA briefly displays a QR-style animation confirming the entry token is valid. The screen auto-advances after roughly 1.4 seconds; tapping anywhere skips immediately. No action is required from the juror.

**What to notice:**

- The animation provides visual confirmation that the link is authentic without exposing any token details.
- Tapping anywhere advances instantly — useful during live presentations when jurors are eager to begin.
...
- A `UserRound` app icon at the top of the card signals the personal-identity context of the form.
...
- The reveal screen includes a clear "Begin Evaluation" confirmation before proceeding — accidental dismissal does not lose the PIN because the admin can look it up in the Jurors page.
