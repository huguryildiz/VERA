...
| 006a | `006a_rpcs_admin.sql` | **Admin RPCs Part A** — sections D (jury mgmt: `rpc_juror_reset_pin`, `rpc_juror_toggle_edit_mode`, `rpc_juror_unlock_pin`), D2 (org admin helpers: `_assert_org_admin`, `_assert_tenant_owner`, `_assert_can_invite`, `rpc_admin_bootstrap` — single-call admin shell hydration returning memberships + organizations + periods + preferred org/default period IDs, `rpc_admin_find_user_by_email`), E (org & token: application approve/reject, `rpc_admin_list_organizations`, `rpc_admin_create_org_and_membership`, `rpc_admin_mark_setup_complete`, period readiness/publish/close, `rpc_admin_generate_entry_token`, unlock request flow: `rpc_admin_request_unlock`, `rpc_admin_list_unlock_requests` (includes `score_count`), `rpc_super_admin_resolve_unlock` (approve deletes all score_sheets for period), `rpc_admin_reopen_period_for_scoring` (lightweight Closed→Live, score-preserving), and related org lifecycle), F (public stats: `rpc_landing_stats`, `rpc_platform_metrics`). |
...
| 008 | `008_platform.sql` | `platform_settings` + `platform_backups` tables; maintenance, metrics, backup CRUD RPCs; auto-backup + maintenance-countdown + `keepalive-warm` cron jobs (the last one runs `rpc_keepalive_warm()` every 10 min to keep bootstrap-relevant query plans + buffer pages hot, cutting `rpc_admin_bootstrap` cold-start from ~6 s to ~200 ms); seeds platform frameworks (MÜDEK v3.1, ABET 2026–2027). **Auto-backup post-deploy:** the `auto-backup-daily` cron reads `vault.decrypted_secrets` for `supabase_url` and `auto_backup_secret`, and the edge function authenticates by matching that secret against the `AUTO_BACKUP_SECRET` env var. After applying 008 to a fresh project, seed both vault entries (`vault.create_secret(...)`) and run `supabase secrets set --project-ref <ref> AUTO_BACKUP_SECRET=<same-value>`. Also confirm `service_role` has `GRANT ALL ON ALL TABLES IN SCHEMA public` — the cron path runs as service role and 002's grants must include it (older snapshots that pre-date this rule still need a one-shot `GRANT ALL ON ALL TABLES IN SCHEMA public TO service_role`). |
...
| `rpc_admin_list_unlock_requests(status)` | List unlock requests filtered by status (`pending`\|`approved`\|`rejected`\|`all`); scoped to caller's org for org-admins, global for super-admins; each row includes `score_count` (number of score_sheets that will be permanently deleted on approval) |
