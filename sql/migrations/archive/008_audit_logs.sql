-- sql/migrations/008_audit_logs.sql
-- Add explicit audit log inserts to key RPCs that were previously unlogged.
-- All functions use CREATE OR REPLACE — safe to apply on any environment.

-- ─────────────────────────────────────────────────────────────────────────────
-- 1. rpc_juror_reset_pin: emit pin.reset on successful reset
-- ─────────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  VALUES (
    ...
    'juror_period_auth',
    p_juror_id,
    jsonb_build_object('period_id', p_period_id, 'juror_id', p_juror_id)
  );
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 2. rpc_jury_finalize_submission: emit evaluation.complete on every finalize
--    (also retains the existing juror.edit_mode_closed_on_resubmit log)
-- ─────────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  -- Fetch org for audit logs (single lookup, used for both inserts below)
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 3. rpc_admin_generate_entry_token: emit token.generate
--    (from 007 base with TTL + new audit log)
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 4. rpc_entry_token_revoke: emit token.revoke
-- ─────────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  VALUES (
    ...
    'entry_tokens',
    p_token_id,
    jsonb_build_object('period_id', v_period_id, 'token_id', p_token_id)
  );
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 5. rpc_period_freeze_snapshot: emit snapshot.freeze on first freeze
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 6. rpc_jury_verify_pin: emit juror.pin_locked when attempts hit the limit
--    (full body from 007 base + new audit log on lockout)
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 7. rpc_juror_unlock_pin: emit juror.pin_unlocked on successful unlock
-- ─────────────────────────────────────────────────────────────────────────────
