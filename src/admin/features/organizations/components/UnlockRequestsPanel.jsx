          ...
          <table className="organizations-table unlock-requests-table table-dense table-pill-balance">
            <thead>
              <tr>
                ...
                <th className={`sortable${unlockSortKey === "score_count" ? " sorted" : ""}`} onClick={() => onUnlockSort("score_count")} style={{ textAlign: "center" }}>Scores at Risk <SortIcon colKey="score_count" sortKey={unlockSortKey} sortDir={unlockSortDir} /></th>
                <th>Reason</th>
            ...
            <tbody ref={unlockScopeRef}>
              ...
              {!unlockLoading && pagedUnlockRows.map((r) => (
                <tr key={r.id} data-status={r.status} data-card-selectable>
                  ...
                  <td data-label="Scores at Risk" className="vera-datetime-text" style={{ textAlign: "center" }}>
                    {r.score_count > 0 ? (
